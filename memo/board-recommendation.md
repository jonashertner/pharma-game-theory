# Strategic recommendation

**To:** Roche Executive Committee
**From:** Strategic-analysis working group
**Date:** 19 May 2026
**Read time:** 6 minutes
**Status:** Pre-board briefing · public-source synthesis · no NDA-protected information

---

## The argument in one paragraph

The December 2025 MFN deal closed a chapter, but the negotiation has not ended — it has shifted from price-setting to *durability engineering*. Roche's single most important strategic asymmetry against every actor at the table is the founding family voting block's 25-year horizon. Every other principal — the Trump administration, US Congress, the Swiss Federal Council, even Novartis under its new chairman — operates on political-cycle or earnings-cycle timeframes. **At a 25-year horizon, the entire analysis flips.** The board's task in 2026 is not to optimize the next four quarters of margin; it is to convert the December framework into multi-decade pharma-pricing infrastructure, while the political coalitions that produced it are still in place.

## Why this is the moment

The model's headline finding is that the May 2026 status quo is *feasible but barely*. The three principals (Roche, US Executive, Swiss FC) sit above their BATNAs; every other actor sits exactly at theirs. The system survives perturbation poorly. Five facts compound the urgency:

1. The Section 232 0% track sunsets 20 January 2029. Renegotiation must close *six to twelve months before* — i.e., in 2028 — to give Roche capex-planning lead time. The window to bake protections into binding form is now.
2. The November 2026 US midterms could remove the political coalition that signed the December deal. An EO is reversible by EO; a USTR bilateral treaty under Senate authority is not. The architecture choice we make in 2026 determines what survives 2028.
3. IRA Round 3 prices for Xolair are effective January 2028. Round 4 selections will be announced in early 2027. The firewall language we put in place this year shapes whether Round 4 selection criteria are constrained by our framework or expand freely.
4. The Celltrion Omlyclo (Xolair biosimilar) launch on 1 September 2026 means our biggest IRA-exposed drug is biosimilar-exposed within months. The framework must not lock us into protections for drugs whose patent calculus is about to invert.
5. The November 2025 Swiss framework granted Switzerland bilateral treatment that the EU did not receive. This is a *gift the Swiss Federal Council needs to consolidate quickly* — they have an independent interest in locking in their own achievement. Bern is our most aligned counterparty for the next 18 months.

## Three moves in 2026

**Move 1 — Negotiate the firewall annex (Q3 2026).** Press for *symmetric* (not Roche-specific) firewall language in the Swiss bilateral annex, modeled on Germany's January 2025 Medical Research Act confidential Erstattungsbetrag mechanism. Frame as cross-cutting OECD-floor architecture, not a Swiss-firm carve-out. Closes before the Section 232 effective date of 31 July 2026; otherwise it becomes a renegotiation, not a clarification. *This is Roche's highest-weight unused lever in the model (10/10 weight on a 3/10 floor) and the only Pareto improvement the simulation finds.*

**Move 2 — Convert the $50B capex pledge into milestone-gated relief (Q4 2026).** Restructure the Section 232 0% track with EPC-style milestone gating — each delivered capex milestone shortens the future tariff-snap-back window. Trades flexibility for predictability and aligns Roche's multi-year capital allocation with the political timeline. **The board should weigh this against the founding family logic:** capex acceleration is acceptable on a 25-year horizon even if it suppresses 2026-2028 ROE.

**Move 3 — Pre-position a tail-risk hedge (Q1 2027 framework proposal).** MFN's worst case is not direct US-channel cuts; it is reference-basket contagion into EU, UK, and Swiss markets via PBM rebate-transparency leaks. The novelty engine's strongest candidate is a reinsurance-style structure: Roche cedes a defined slice of US revenue above a price floor to a confidential pricing-protection facility, in exchange for capped downside if benchmarks shift globally. Unconventional but analytically clean and politically deniable.

## Decisions before the board

Four asks. Two require decision; one is discretionary; one is a note.

(a) **Endorse the firewall-annex approach** as the primary 2026 negotiating priority — explicitly above MFN-coverage expansion or TrumpRx SKU growth. *Decision required.*

(b) **Authorize capital-allocation flexibility** for capex milestone acceleration (up to +$5B over the 2025–2027 window) to enable Move 2 without quarterly-cycle constraint. *Decision required.*

(c) **Defer the patient-access out-of-pocket cap commitment** until Q4 2026, pending CMS rulemaking signals. *Defer or commit early at executive discretion.*

(d) **Note the tail-risk hedge structure** (Move 3) as an exploratory workstream for Q1 2027 framework adoption. *Note only.*

## The generationalist re-framing

Standard pharma-policy analysis treats the December 2025 framework as a 2026–2029 contract. That framing is correct for the US Executive (whose horizon is the 2028 election), correct for US Congress (whose horizon is the 2026 midterms), correct for institutional Roche investors (whose horizon is the next three quarterly cycles), correct for the Swiss Federal Council (whose horizon is the 2027 federal elections).

It is *incorrect for the family-held voting bloc* , whose horizon is multi-generational, and whose Roche board appointments (Schwan re-elected 97.75% in March 2026) confirm this. The family logic accepts trade-offs no public-board-driven company can accept:

— Lower 2026–2028 ROE for 2030+ pipeline value.
— Capex acceleration to lock in regulatory protection ahead of the cycle.
— Quiet engagement with EU Commission strategic-autonomy leadership on multi-decade pharma-innovation-infrastructure — a conversation impossible at corporate-Roche level because it would look self-interested, but appropriate at family-stewardship level.
— Cultivation of Basel cantonal cluster preservation as a parallel stewardship logic, with periodic Basel–family–Bern coordination forums.

**The board should evaluate the three moves on a 25-year payoff, not a 2-year one.** At that horizon, the firewall annex is not a tactical concession; it is the construction of multi-decade pricing-protection infrastructure. The capex pledge is not an FY26 commitment; it is a 50-year cluster bet. The tail-risk hedge is not an exotic hedge; it is the prudent equivalent of the reinsurance contracts that have protected family wealth across centuries.

## Three unexpected alliances the standard frame misses

1. **Roche and the Trump administration share an interest in *treaty form* over executive-order form.** Roche wants durability; the administration wants legacy. Both lose if MFN is erased by a successor EO in 2029. Standard pharma-policy framing assumes the administration prefers flexibility, but a flagship achievement is more valuable as legislation or treaty than as a directive. Frame the firewall annex as preserving the December 2025 achievement against political reversal. USTR is the right counterparty, not HHS.

2. **The founding family and the EU Commission share a multi-decade interest in European pharma-innovation-infrastructure persistence.** At 2-3 year horizons they conflict (prices). At 20+ year horizons they are aligned (cluster survival). The Critical Medicines Act (provisional 12 May 2026) signals that the Commission has accepted higher EU prices in exchange for retained EU manufacturing — same logic as the family's. *This conversation is impossible at Roche-corporate level; it must happen at family-ownership level.* Recommend the chairman initiate periodic, low-profile dialogue with DG SANTE strategic-autonomy leadership.

3. **Basel-Stadt cantonal government and the founding family share cluster-preservation telos at different scales.** They never appear in the same political room but their interests are nearly identical. Formal Basel–family–Bern coordination — quarterly informal, semi-annual structured — would lock in the long-horizon framing against short-cycle pressures from Bundestag-style noise.

## Contradictions to exploit (not resolve)

— The Trump administration's MFN program is durable only in treaty form, but MAGA philosophically rejects treaties. **Exploit:** offer treaty form to the administration's *legacy-conscious* faction (USTR, HHS career staff, Treasury) rather than its *political-cycle* faction (White House communications). The legacy-conscious faction will spend political capital we don't have to spend.

— EU industrial policy wants pharma prices low (G-BA/NICE methodology) AND high enough to retain EU manufacturing (Critical Medicines Act). **Exploit:** the firewall annex's confidential-net-price mechanic resolves this tension in the EU's favor without the EU having to choose publicly. The Commission can claim the framework as their methodology in disguise.

— PBMs cannot publicly defend their information-arbitrage business model. **Exploit:** the firewall annex distinction between US-channel list-price transparency (settled) and ex-US net-price confidentiality (at stake) is one the PBM lobby will naturally defend in public. Let them carry the case while Roche stays out of the PBM debate.

## Where this could be wrong

Three structural assumptions could fail.

— *That MFN-basket contagion to EU/UK/CH is the dominant risk.* If direct US-channel margin compression dominates instead (via visible-list-price politics or TrumpRx expansion), the firewall is mis-prioritized. The model cannot fully distinguish these because confidential PBM net prices are not observable. **Mitigation:** track the Cigna / Express Scripts rebate-free pivot (Oct 2025) and the Caremark insulin litigation outcome for directional signal.

— *That the November 2026 US midterms preserve the current executive coalition.* If they do not, the December 2025 framework may be re-opened. **Mitigation:** the firewall annex must be a binding bilateral treaty instrument under USTR authority *before* the midterms. Treaty form survives administration turnover; an executive-action annex does not.

— *That the Swiss Federal Council can carry the cantons and the domestic-payer constituency.* The model splits the Swiss side into three actors precisely because the federal trade negotiator does not automatically carry Basel cantonal interests or domestic-payer interests. If Roche's preference for the firewall leaks into raising Swiss domestic prices (Schinecker's October 2025 ask), Bern loses the cantons. **Mitigation:** explicit zero-anchor on swiss_domestic_pricing in any annex — already in the model's recommended deal vector. No Roche executive should publicly propose raising Swiss prices to subsidize US benchmarks while the firewall annex is being negotiated.

## What this memo doesn't argue

This memo recommends Moves 1, 2, 3 *in addition to* the existing December 2025 commitments, not in place of them. It does not advocate for reopening MFN coverage (politically infeasible and analytically unnecessary). It does not propose direct opposition to IRA Round 3 Xolair pricing (statutorily settled; not contestable through trade-deal channels). It does not address the longer-horizon question of biosimilar competition after the 2029 Ocrevus patent cliff — that is a separate strategic question that should be addressed by clinical and formulation defense (the next-generation MS therapeutic — trontinemab readouts 2028+), not by legal action against biosimilars.

The supporting analysis — twelve sourced actor profiles with full telos analysis, a thirteen-issue payoff model, an eighteen-event simulation transcript, and fifteen analogue-domain collisions for novel-move generation — is one click away. The model is calibrated against the actual signed December 2025 state, so the model cannot have predicted Roche should have walked from a deal it has already signed; that test passes.

The board is asked to act on (a) and (b) before the August 2026 negotiation window closes.

---

*Supporting reading: [strategic dynamics — telos, contradictions, alliances](dynamics.html) · [the live simulation](simulation.html) · [12 actor profiles](actors.html) · [13-issue playground](playground.html) · [novelty engine](prompts.html) · [methodology and calibration](method.html) · [generated analytical detail](recommendation-detail.html)*
